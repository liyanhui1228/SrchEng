...
public class QrySopOr extends QrySop {
  ...
  /**
   * Implemented the abstract method getRankedScore in the QrySop class
   * Get the max matched score in the query arguments for OR operator
   * @return The document score
   * @throws IOException
   */
  @Override
  public double getRankedScore(){
	  ...
	  for(int i=0; i<this.args.size(); i++){
		  ...
		  if(this.docIteratorHasMatchCache() && matched_docID == this.args.get(i).docIteratorGetMatch()){
			  if(qrysop_cur.getRankedScore() > max_score){
				  max_score = qrysop_cur.getRankedScore();
			  }
		  }
	  }
	  return max_score;
